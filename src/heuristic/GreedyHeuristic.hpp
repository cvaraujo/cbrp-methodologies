...
public:
    ...
    double SolveScenario(vector<double> cases, vector<int> time, int T, vector<int> &y);

    Solution Run(double route_time_increase, int max_tries, bool use_avg);

    double BinarySolve(vector<double> cases, vector<int> time, int reserved_time, int T, vector<int> &y);
};
