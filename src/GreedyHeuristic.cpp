...
float GreedyHeuristic::Run(float route_time_increase, int max_tries, vector<vector<pair<int, int>>> &sol_x, vector<vector<pair<int, int>>> &sol_y)
{
    ...
    for (int s = 0; s < S; s++)
    {
        ...
        for (int i = 0; i < B; i++)
        {
            cases[i] = graph->scenarios[s].probability * graph->scenarios[s].cases_per_block[i];
            if (in_first_stage[i])
                cases[i] = cases[i] * (1 - alpha) * graph->scenarios[s].probability;
        ...
        if (!all_zeros)
            of += SolveScenario(cases, time, route_time_increase, max_tries, y[s + 1], x[s + 1]);
    }
    ...
    for (int s = 0; s < S + 1; s++)
    {
        ...
        for (auto i : x[s])
            ...
            {
                sol_y[s].push_back({i.first, i.second});
